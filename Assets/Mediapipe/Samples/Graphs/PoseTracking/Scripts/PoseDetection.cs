...
public class PoseDetection : MonoBehaviour
{
    ...
    PoseTrackingGraph poseTrackingGraph;
    ...
    void Start()
    {
        //GameObject PoseTrackingObject = GameObject.Find("PoseTracking(Clone)");
        poseTrackingGraph = transform.root.gameObject.GetComponent<PoseTrackingGraph>();
        Debug.Log("pose graph found: " + poseTrackingGraph);
    ...
    void Update()
    {
        if (poseTrackingGraph.poseLandmarks != null)
        {
            Debug.Log("LAAAANNDDDMARRRKRKKRKRKRKR: " + poseTrackingGraph.poseLandmarks);
            for (int i = 0; i < poseTrackingGraph.poseLandmarks.Landmark.Count; i++)
            {
                inputs[0, i, 0] = poseTrackingGraph.poseLandmarks.Landmark.ElementAt(i).X;
                inputs[0, i, 1] = poseTrackingGraph.poseLandmarks.Landmark.ElementAt(i).Y;
                inputs[0, i, 2] = poseTrackingGraph.poseLandmarks.Landmark.ElementAt(i).Z;
                inputs[0, i, 3] = poseTrackingGraph.poseLandmarks.Landmark.ElementAt(i).Visibility;
